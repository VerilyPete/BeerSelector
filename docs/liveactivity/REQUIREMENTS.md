...
- **Trigger Events**:
  ...
  - App foregrounded (sync check)
- **Update Mechanism**:
  - **Local updates only**: Direct `Activity.update()` calls from React Native bridge
  - **No push notifications**: Updates occur only when app is active
...
- **Mechanism**: Local `Activity.update()` via React Native bridge
...
**FR-3.2: App Foreground Sync**

- **Mechanism**: Check queue state when app becomes active
...
- **Note**: External changes only reflected when user opens the app
...
- **App Group**: `group.org.verily.FSbeerselector.shared` (for data sharing between app and widget)

**TR-2.2: No Push Notifications Required**

- This implementation uses **local updates only**
- No APNs certificate needed
- No backend push service required
- Updates occur via direct `Activity.update()` calls when app is active

**Note**: Beer names should be stripped of container type (e.g., "(Draft)", "(BTL)") before updating Live Activity.
...
**TR-5.3: Update While App Backgrounded**

- **Scenario**: External queue change (employee confirmation) while app is closed
- **Behavior**: Live Activity shows stale data until app is opened
- **Recovery**: Sync on next app foreground
- **Note**: This is expected behavior for local-only updates
...
### Q-2: Background Updates (Future Enhancement)

**Question**: Should we add background polling or push updates in a future version?
**Impact**: Low - current local-only approach is acceptable for V1
**Alternatives**:

- Keep local-only (simple, no backend changes)
- Add APNs push for real-time updates (requires backend)
  **Decision**: Deferred to V2 based on user feedback
...
**Risk**: Live Activity shows outdated queue when app is backgrounded
**Probability**: Medium (expected behavior with local-only updates)
**Impact**: Low - users understand they need to open app for fresh data
**Mitigation**:

- Sync queue state on every app foreground
- Clear messaging that updates require opening the app
- Consider adding push notifications in future version if needed
...
- [ ] Set up Live Activity widget extension in Xcode
- [ ] Create `liveActivityService.ts` abstraction layer
- [ ] Implement start/update/end functions via React Native bridge
...
### Phase 3: UI Polish & Theming (1 week)
...
- [ ] Manual testing on multiple iOS versions (16.1, 16.2, 17.x, 18.x)
