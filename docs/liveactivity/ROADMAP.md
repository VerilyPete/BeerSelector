...
- [ ] **Setup APNs Certificate**
  ...
  - Download `.p8` file and save Key ID + Team ID
  **Estimated**: 1 hour

- [ ] **Configure EAS Build**
  - Update `eas.json` for development builds
  - Add APNs credentials to EAS
  **Estimated**: 30 minutes

**Deliverables**:
- Configured project with Live Activity support
- APNs certificate ready for use
...
- [ ] **Create Development Build**
  ...
  ```
  **Estimated**: 30 minutes (build time: 15-20 minutes)
...
**Deliverables**:
- Working dev client on physical device
- Verified existing functionality intact

**Blockers**:
- EAS Build service availability
- iOS device with iOS 16.1+
...
- [ ] **Release Build**
  ...
  - Set release date
  **Estimated**: 1 hour (build time: 20-30 minutes)
...
| APNs push delivery issues | Low | Medium | Implement periodic polling fallback | Rely on polling only, notify users of reduced real-time updates |
| Swift widget crashes on older iOS versions | Low | High | Test on iOS 16.1, 16.2, 17.0+ | Add iOS version checks, disable for problematic versions |
| Dev client build failures | Medium | Medium | Use EAS Build cloud service | Local builds with Xcode as backup |
...
| Xcode/Apple Developer issues | Medium | Low | Document setup process, use EAS Build | Pair with iOS expert, escalate to Expo support |
...
- [ ] Apple Developer account with APNs access
...
- [ ] Basic Swift knowledge (or access to Swift developer)
- [ ] iOS development toolchain familiarity
...
- **APNs**: Free (included with Apple Developer account - $99/year)
- **EAS Build**:
  - Free tier: Limited builds/month
  - Priority builds: $29/month (recommended)
...
// In code:
if (features.liveActivity.enabled &&
    !isVisitorMode &&
    await isLiveActivitySupported()) {
  await startLiveActivity(queueState);
}
```
...
**Configuration**:
- `app.json` - Expo config with Live Activity plugin
...
# Run locally (after dev client installed)
npm start
