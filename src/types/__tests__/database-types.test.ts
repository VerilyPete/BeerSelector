...
import { Preference, Reward, UntappdCookie } from '../database';
...
describe('Database Type Guards', () => {
  ...
  describe('isUntappdCookie', () => {
    it('should return true for valid UntappdCookie objects', () => {
      const validUntappdCookie: UntappdCookie = {
        key: 'test-key',
        value: 'test-value',
        description: 'Test description'
      };
      
      expect(isUntappdCookie(validUntappdCookie)).toBe(true);
    });
    ...
    it('should return false for invalid UntappdCookie objects', () => {
      ...
      expect(isUntappdCookie(missingValue)).toBe(false);
      expect(isUntappdCookie(missingDescription)).toBe(false);
      expect(isUntappdCookie(wrongTypes)).toBe(false);
      expect(isUntappdCookie(null)).toBe(false);
      expect(isUntappdCookie(undefined)).toBe(false);
    });
  });
});
