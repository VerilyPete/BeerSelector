...
describe('Environment Variable Configuration', () => {
  ...
  describe('EXPO_PUBLIC_ Environment Variables', () => {
    it('should load API base URL from EXPO_PUBLIC_API_BASE_URL', async () => {
    ...
    it('should load development base URL from EXPO_PUBLIC_DEV_API_BASE_URL', async () => {
    ...
    it('should load staging base URL from EXPO_PUBLIC_STAGING_API_BASE_URL', async () => {
    ...
    it('should load production base URL from EXPO_PUBLIC_PROD_API_BASE_URL', async () => {
    ...
    it('should load network timeout from EXPO_PUBLIC_API_TIMEOUT', async () => {
    ...
    it('should load retry count from EXPO_PUBLIC_API_RETRIES', async () => {
    ...
    it('should load default environment from EXPO_PUBLIC_DEFAULT_ENV', async () => {
    ...
    it('should load Untappd base URL from EXPO_PUBLIC_UNTAPPD_BASE_URL', async () => {
  ...
  describe('Fallback to Defaults', () => {
    it('should use hardcoded default when env var not set', async () => {
    ...
    it('should use default timeout when EXPO_PUBLIC_API_TIMEOUT not set', async () => {
      const { config } = await import('../config');

      expect(config.network.timeout).toBe(15000); // Default from config.ts
    });

    it('should use default retries when EXPO_PUBLIC_API_RETRIES not set', async () => {
      const { config } = await import('../config');

      expect(config.network.retries).toBe(3); // Default from config.ts
    });

    it('should use default environment when EXPO_PUBLIC_DEFAULT_ENV not set', async () => {
      const { config } = await import('../config');
  ...
  describe('Environment Variable Validation', () => {
    it('should validate URL format for EXPO_PUBLIC_API_BASE_URL', async () => {
    ...
    it('should validate numeric value for EXPO_PUBLIC_API_TIMEOUT', async () => {
    ...
    it('should validate environment name from EXPO_PUBLIC_DEFAULT_ENV', async () => {
    ...
    it('should remove trailing slash from env var URLs', async () => {
      process.env.EXPO_PUBLIC_PROD_API_BASE_URL = 'https://test-api.example.com/';
  ...
  describe('Multiple Environment Variables', () => {
    it('should load all environment-specific URLs when all env vars set', async () => {
  ...
  describe('Priority Order', () => {
    ...
    it('should fall back to generic URL if environment-specific not set', async () => {
    ...
    it('should use hardcoded default if no env vars set', async () => {
      // Don't set any env vars
      const { config } = await import('../config');
      config.setEnvironment('production');
  ...
  describe('Runtime Environment Switching', () => {
    it('should use correct URL after switching environments with env vars', async () => {
      process.env.EXPO_PUBLIC_DEV_API_BASE_URL = 'https://dev.example.com';
      process.env.EXPO_PUBLIC_PROD_API_BASE_URL = 'https://prod.example.com';
    ...
    it('should maintain custom API URL override despite env vars', async () => {
  ...
  describe('Type Safety with Environment Variables', () => {
    it('should convert string env vars to numbers for numeric config', async () => {
    ...
    it('should handle boolean-like env vars', async () => {
  ...
  describe('Environment File Loading', () => {
    it('should document which .env file patterns are supported', async () => {
