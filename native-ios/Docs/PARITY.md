...
### Walkthrough fixes — 2026-09-12

Queue now has a chrome header and explicit Delete/Cancel alert, visible loading/failure/retry states and a confirmed-empty state. Existing rows remain visible after failed refresh, with stale-data wording; deletion failures remain in the sheet. Account epoch guards protect queue feedback. Expanded Finder amber actions now match the accepted subdued Queue/Rewards materials. Inactive tabs remain accepted and unchanged. 107/107 correctness tests passed; offline normal/maximum-text UI checks passed, with no live account submissions. See current WORK-CHECKPOINT.md for logs.
