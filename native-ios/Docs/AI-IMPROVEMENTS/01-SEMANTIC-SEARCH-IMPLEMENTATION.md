...
Settings and Suggestions share a live status derived from the same check used by the actual recommendation provider. It reports Apple Intelligence availability or local matching, distinguishing an older OS, ineligible device, Apple Intelligence being disabled, model-not-ready, unsupported locale and future unknown unavailability reasons. The check runs on foregrounding and every 15 seconds while the status is active; it does not run inference. An available model is not a promise of successful generation: the status explains fallback, and the existing per-result label continues to report whether Apple Intelligence actually selected that batch.

Compatibility-status validation: all 222 tests passed in Release on both iOS 27 and iOS 26.3, including availability-reason and locale mapping plus the real-provider smoke test. Bundles: `/private/tmp/compatibility-status-27.xcresult` and `/private/tmp/compatibility-status-26-3.xcresult`. `git diff --check` passed.
