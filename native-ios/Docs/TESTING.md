...
## Credential-write failures — 2026-09-11

CredentialFailureTests uses per-instance OS write-call injection, returning errSecInteractionNotAllowed for selected Keychain keys while allowing all other operations through to real simulator Keychain storage. Each test owns a unique namespace, SQLite file and HTTPFixture. Six tests exercise nine scenarios: four save stages with an existing account, all four on first login, and an auto-login failure. A second cookie chunk is rejected after an earlier chunk has been written, checking partial generation behavior.

The old login path failed four tests: it unnecessarily rewrote still-valid credentials during rollback, and persistent Keychain failure prevented restoring the old data URL. Login now distinguishes pre-commit failures from successful credential saves followed by database errors. Tests verify unchanged committed generation/session/cookies, preferences, cached lists/rewards and pending operations, preserved in-memory account state, no follow-up requests, and recovery through a later successful save. Full suite **62/62 passed**; red log `/private/tmp/BeerSelectorNative-credentials-red.log`, green log `/private/tmp/BeerSelectorNative-credentials-all.log`. Integration and All include the cases. Failed rollback after a successful Keychain commit plus a database failure, OS read/delete failures and abrupt process termination between stores remain outside this coverage.

## Database failure after credential commit — 2026-09-11

Three LoginRollbackTests exercise successful existing-account rollback, first-login rollback, and failed Keychain rollback following a real SQLite trigger failure during account cache replacement. The trigger aborts after earlier deletes; reopening the database confirms the old cache and pending operations survive. The combined-failure regression initially failed because old member data could appear under the newly persisted credentials on a fresh model.

A persistent account-transition flag now gates startup credential restoration until login or rollback finishes. Tests verify the current model becomes signed out after rollback failure, fresh credential restoration rejects the pending transition, member lists remain hidden, another failed login cannot remove the guard, and a later successful login replaces the cache and clears the flag. Startup and tests use the same restoreCredentials method. No actual process kill or live backend is involved.

Full suite **65/65 passed**: `/private/tmp/BeerSelectorNative-rollback-all.log`; red regression `/private/tmp/BeerSelectorNative-rollback-red.log`. Integration and All include LoginRollbackTests. This extends the earlier credential-write coverage to the combined database/Keychain rollback case; actual power-loss durability, arbitrary OS read/delete failures and further logout/login interleavings remain unverified.

## Overlapping logout/login — 2026-09-11

LogoutRaceTests covers login waiting for old local cleanup, a late old-session server-logout failure, and a second logout invalidating login while it waits for cleanup. Per-model cookie-cleanup injection controls the async boundary; real uniquely namespaced Keychain and SQLite check the resulting account/configuration, including a fresh model. HTTP remains entirely fixture-based. An inverted expectation checks that login cannot publish during suspended cleanup; continuations control release. These tests do not depend on real WebKit latency.
...
## Legacy schema-v8 compatibility — 2026-09-11

LegacyV8Fixture snapshots the exact six table definitions from src/database/schema.ts and schemaVersion.ts at 363ac3ef, plus the two operation-queue indexes. Synthetic beer/tasting enrichment and review fields, rewards, settings, migration history and pending/retrying/failed/success operation rows are inserted using raw SQLite before any native initialization. Both fresh-v8 and migrated-v8 history layouts are checked. If the reference schema changes, review/update the fixture deliberately; it is independent of native setup and does not prove compatibility with unknown future schema versions.

Three LegacyUpgradeTests verify preservation across repeated opens (including original columns, preferences/descriptions, indexes and migration history), native refresh-style writes/enqueue compatibility, and atomic rollback/retry when a SQLite trigger aborts the last setup write. The test checks all operation payload/retry metadata, allowing only the intentional retrying-to-pending recovery. Successful queue rows remain stored but are excluded from native pending-operation reads.

Full suite **74/74 passed**, log `/private/tmp/BeerSelectorNative-legacy-all.log`; no production change needed. The initial focused run exposed a row-ordering error in the test projection, not lost preferences. Integration and All include these tests. This is an exact-schema fixture upgrade check, not a real prior-binary installation/update test or exhaustive historical-version certification. Separate Expo Keychain format tests remain in PersistenceIntegrationTests.
