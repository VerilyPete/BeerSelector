...
struct BeerListScreen: View {
    ...
    var body: some View {
        ...
        }.padding(.horizontal,18).padding(.top,8).frame(maxWidth:960).frame(maxWidth:.infinity)
    ...
    private var queueControls: some View {
        Group {
            ...
            }.buttonStyle(BeerControlStyle(appearance:.amber,compact:true)).disabled(model.loadingQueue).accessibilityIdentifier("view-queue-button")
    ...
    var body: some View {
        ChromePanel(padding:10) {
            VStack(alignment:.leading,spacing:10) {
                Button(action:toggle) {
                    HStack(spacing:10) {
                        ...
                        VStack(alignment:.leading,spacing:3) {
                            Text(beer.brew_name).font(Robo.title()).foregroundStyle(Robo.cyan).lineLimit(expanded ? nil : 1).accessibilityIdentifier("beer-name-\(beer.id)")
                            Text(beer.brewer + (beer.tasted_date.isEmpty ? "" : " · \(dateLabel) \(beer.displayDate)")).font(Robo.mono(10)).foregroundStyle(Robo.steel).lineLimit(expanded ? nil : 1)
                            Text([beer.brew_style,beer.brew_container].filter { !$0.isEmpty }.joined(separator:" · ")).font(Robo.mono(10)).foregroundStyle(Robo.steel).lineLimit(1)
                        }.frame(maxWidth:.infinity,alignment:.leading)
                ...
                }.buttonStyle(.plain).accessibilityIdentifier("beer-item-\(beer.id)").accessibilityValue(expanded ? "Expanded" : "Collapsed")
                if expanded {
                    ...
                    HStack {
                        if checkIn {
                            Button { Task { await model.checkIn(beer) } } label: { if model.busyIDs.contains(beer.id) { ProgressView().tint(Robo.amber) } else { Text(pending ? "QUEUED" : "CHECK IN") } }.buttonStyle(BeerControlStyle(appearance:.amber)).disabled(pending || model.busyIDs.contains(beer.id)).accessibilityIdentifier("check-in-\(beer.id)")
                        }
